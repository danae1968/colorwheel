function [data] = showChoiceTrialFixedDUTCH(pms, data, wPtr,rect, dataFilenamePrelim)
...
%Break after every block
    if trial~=length(data.block)
        if data.block(trial)-data.block(trial+1)~=0
            if pms.practice==0
                DrawFormattedText(wPtr,sprintf('Dit is het einde van blok %d, druk op een toets om verder te gaan.',data.block(trial) ),'center','center',[0 0 0]);
                Screen('Flip',wPtr)
