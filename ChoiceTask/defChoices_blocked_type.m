function [data]=defChoices_blocked_type(pms)
...
%blocking choice task such that e.g. 6 UPD trials; then 6 IGN trials
IGN = repmat((1:3)',pms.nCalTrials,1); %index for IGN
UPD = repmat((4:6)',pms.nCalTrials,1); %index for UPD
...
if rand < 0.5
    block(mod(block,2)==0,2) = idx_IGN;
    block(mod(block(:,1),2)==1,2) = idx_UPD;
else
    block(mod(block,2)==0,2) = idx_UPD;
    block(mod(block(:,1),2)==1,2) = idx_IGN;
end
