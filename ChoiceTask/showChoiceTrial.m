function [data] = showChoiceTrial(pms, data, wPtr,rect,dataFilenamePrelim)
...
for trial = 1:length(data.trialNumber)
    ...
    switch data.locationEasy(trial)
        ...
        case 2 %means easy option right, hard left
            [msgTypeL,msgTypeR]=deal(msgTypeR,msgTypeL);
...
%         imwrite(imageArray,sprintf('ChoiceDUTCH%d.png',trial),'png');
...
%     while (GetSecs - offerOnset) < pms.maxRT
        % check keyboard
        RestrictKeysForKbCheck([pms.allowedResps.left, pms.allowedResps.right, 97, 98])
        [ secs, keyCode] = KbWait;
        WaitSecs(0.1);
...
%                 imwrite(imageArray,sprintf('ChoiceMade%dDUTCH.png',trial),'png');
    ...
    if isempty(responded)
        resp = NaN;
        choiceRT = NaN;
        DrawFormattedText(wPtr,'Reageer sneller!','center',ylength*(1/4));
                Screen('Flip',wPtr)

        WaitSecs(pms.fb);
        
        if data.tskAmtTrlNm(trial) < pms.nCalTrials
            i = find(data.typeTask==data.typeTask(trial) & data.tskAmtTrlNm == data.tskAmtTrlNm(trial)+1);
...
%Break after every block
            ...
            save(fullfile(pms.choicedir,dataFilenamePrelim.dataFilenamePrelim)) % save after every trial in prelim file
